from PyQt5.QtWidgets import QMainWindow, QApplication, QWidget, QGridLayout, QPushButton, QDialog, QFrame, QLabel, \
    QToolButton, QFileDialog, QGraphicsDropShadowEffect
...
# from LocalFuse import bookIn
# from newLocalFuse import bookIn
...
from LocalCut import bookSearch
from InternetCut import bookBorrow

from VideoCut import bookReturn

from setup import cardManage
...
class bookSystem(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.effect_shadow.setOffset(2.5,2.5)
    ...
    def init_left(self):
        ...
        # 绑定点击事件，
        """ left_widget中的left_close被点击了，链接到closeWindow函数，关闭对应的窗口"""
        ...
        # 绑定信号和槽
        """left_widget中的left_button1（图书查询按钮）绑定图书查询函数，"""
        ...
        # TUDO：创建设置界面和信号绑定
        """这里面实现不同页面的转换是通过隐藏其他的页面，只显示当前，槽函数点击创建的页面"""
    ...
    def main_view(self):
        ...
        introduction = '''
               <div style="text-align:center; font-size: 60px;font-family:'Microsoft Yahei'; color:#F76677;"><b>图像智能分割与融合系统</b></div> 
               <div style="text-align:center; font-size: 15px;font-family: 'Arial'; color:#F76677;"><b></b><div>
               <div style="text-align:center; font-size: 15px;font-family: 'Arial'; color:#F76677;"><b></b></div>
           '''
    ...
    def return_mainView(self):
        ...
        self.bookIn_widget.hide()#图书入库
        self.bookSearch_widget.hide()#图书查询
        self.bookBorrow_widget.hide()#借阅书籍
        self.bookReturn_widget.hide()#返回书籍
        self.cardManage_widget.hide()#借阅证
...
def windows_main():
    with open('./windows/app.qss', encoding='utf-8') as f:
        qss = f.read()
    app = QApplication(sys.argv)
    app.setStyleSheet(qss)
    gui = bookSystem()
    gui.setWindowIcon(QIcon('.windows/img/Title.ico'))
    # 要改
    gui.show()
    sys.exit(app.exec_())
